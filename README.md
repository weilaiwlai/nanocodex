...
nanocodex 是一个面向**本地代码仓库**的智能编程助手，基于 OpenAI Agents SDK 构建。与云端 Code Agent 不同，nanocodex 深度整合本地环境，具备 workspace 级长期记忆和精细化的上下文治理能力，让 AI 能够真正理解你的代码库，成为你日常开发中的可靠搭档。

## 核心亮点

### workspace 级长期记忆
nanocodex 为每个项目维护独立的记忆空间。首次分析代码库后，重要信息会被持久化存储，后续对话无需重复解释项目背景，AI 能够"记住"你的代码结构和开发习惯。这意味着：
- 跨会话保持对项目的理解，避免重复上下文构建
- 自动学习项目的代码风格、架构模式和依赖关系
- 针对特定项目的建议更加精准和个性化

### 自适应上下文治理
长会话是代码助手的常见痛点。nanocodex 通过 L1 / L2 / L3 分层压缩策略，自动管理上下文长度，在保持对话连贯性的同时避免信息膨胀。
...
### 灵活的 Session 管理
支持创建可命名、可选择、可恢复的会话。你可以为不同任务开启独立 session，也可以在任意时刻中断并后续接续。session 可绑定特定项目目录，也可以共享当前工作目录。Agent Team 中的每个 teammate 拥有独立 SQLiteSession，支持会话级持久化和重启恢复。
- 支持 `--workspace <path>` 显式绑定项目目录
- 未指定 workspace 时，默认绑定当前 shell 工作目录
- session 状态持久化，支持跨终端恢复和切换
- teammate 会话可跨进程恢复，未完成任务自动重启

### Skills 扩展机制
通过 `SkillLoader` 和 `Skill` 协议，nanocodex 支持动态加载自定义技能。目前内置了 code-review 和 repo-explore 两个 skill，你可以根据需要扩展更多场景。
- **code-review**：自动分析代码质量、潜在 bug 和性能问题
- **repo-explore**：快速扫描仓库结构，生成项目概览和关键模块说明
- 支持通过简单的配置文件定义新 skill，无需修改核心代码

### 仓库规则 (Repo Rules)
nanocodex 支持通过 `.nanocodex/rules/` 目录注入项目级别的代码规范和约束。每个 `.md` 文件作为一个独立规则 section，自动注入到 Agent 的上下文中。也兼容项目根目录的 `code_law.md` 单文件配置。
...
# 创建并激活 conda 环境
conda create -n nanocodex python=3.12 -y
conda activate nanocodex

# 安装依赖
pip install openai>=2.29.0 openai-agents>=0.13.0 prompt-toolkit>=3.0.52 python-dotenv>=1.2.2 tiktoken>=0.9.0
...
> **Conda 用户注意：** 以下所有 `uv run python` 命令替换为 `python` 即可（需先 `conda activate nanocodex`）。

```bash
# 从仓库根目录启动 CLI
uv run python scripts/cli.py          # uv 用户
python scripts/cli.py                 # conda 用户
...
- **tasks**：支持复杂任务的分解和执行，包括子代理（SubAgent）、后台执行（BackgroundRun）和 Agent Team 协作。Agent Team 支持独立 session、重启恢复、worktree 分支隔离与 merge。
...
│   ├── context/                  # 上下文工程
│   │   ├── __init__.py
│   │   ├── context_builder.py    # 上下文构建器，L1/L2/L3 分层构建，支持 .nanocodex/rules/ 多文件
...
│   │   ├── agent_team.py          # 多 Agent 协作：team-lead、独立 session teammate、重启恢复
│   │   ├── subagent.py            # 一次性分析子代理
│   │   ├── background.py          # 后台命令执行
│   │   └── worktrees.py           # Git worktree 管理：分支隔离、merge
│   │
│   ├── protocol/                 # 工具协议
│   │   ├── __init__.py
...
│   │   └── <session-id>/
│   │       └── team/
│   │           └── sessions/    # Teammate 独立 SQLiteSession
...
| L2 | 仓库规则 + 长期记忆 | `.nanocodex/rules/` 多文件规则（回退 `code_law.md`）+ workspace-scoped memory |
| L3 | Session Summary | 会话摘要，包含目标、约束、文件、待办、主题、错误、决策理由 |
...
- **workspace 记忆**：存储在 `~/.nanocodex/projects/<project-key>/memory/` 目录，包含项目结构、关键代码和历史交互信息。
- **工具输出**：工具执行的完整输出会自动落盘，可通过回查命令获取。

### 日志级别
nanocodex 支持多级别日志，可在 `.env` 文件中配置：
...
| 特性 | nanocodex | 云端 Code Agent | 本地 IDE 插件 |
|------|-------|----------------|---------------|
| 本地执行 | ✅ | ❌ | ✅ |
| 长期记忆 | ✅ | ❌ | ❌ |
| 上下文治理 | ✅ | 有限 | 有限 |
...
### 已完成 (Phase 4)
- ✅ Teammate 独立 SQLiteSession，支持会话级持久化
- ✅ Team 重启恢复：未完成任务自动重启 teammate
- ✅ Worktree branch-per-task + merge workflow
- ✅ Span Transcript：支持 span 层级和执行时长追踪
- ✅ TodoState 增加 id 追踪
- ✅ 仓库规则多文件支持（`.nanocodex/rules/`）
- ✅ 上下文压缩结构化输出扩展
...
nanocodex 仍在持续迭代中，欢迎提出问题和改进建议。如果你对项目感兴趣，欢迎参与贡献！